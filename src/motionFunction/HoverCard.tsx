...
const HoverCard: React.FC<Props> = ({
    ...
    const [cursorCoods, setCursorCoors] = useState({ x: 0, y: 0 });
    ...
    const translateX =
        typeof window !== "undefined"
            ? calcTranslate(cursorCoods.x, window.innerWidth, 600)
            : 0;
    const translateY =
        typeof window !== "undefined"
            ? calcTranslate(cursorCoods.y, window.innerHeight, 500)
            : 0;
    ...
    return (
        ...
        >
            <div
                className={`z-[1] relative bg-[#161b22] h-full border-[0.5px] rounded-xl shadow-xl md:flex ${direction} justify-between`}
            >
                {children}

                <div className={`absolute w-[500px] border-none bottom-[50px] h-[1000px] z-[-1] back ${hovered ? "opacity-95" : "opacity-0" }`}
                    style={{
